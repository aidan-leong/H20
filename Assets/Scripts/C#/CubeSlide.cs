...
public class CubeSlide : MonoBehaviour {
	...
	void Update() {
		xInt = (int)transform.position.x;
		yInt = (int)transform.position.y;
		zInt = (int)transform.position.z;

		if (Input.GetKeyDown(KeyCode.I) == true && canMove == true) {
			canMove = false;
			StartCoroutine(MoveInGrid(xInt, yInt, zInt + gridSize));
			AddStepCount();
		}
		if (Input.GetKeyDown(KeyCode.L) == true && canMove == true) {
			canMove = false;
			StartCoroutine(MoveInGrid(xInt + gridSize, yInt, zInt));
			AddStepCount();
		}
		if (Input.GetKeyDown(KeyCode.J) == true && canMove == true) {
			canMove = false;
			StartCoroutine(MoveInGrid(xInt - gridSize, yInt, zInt));
			AddStepCount();
		}
		if (Input.GetKeyDown(KeyCode.K) == true && canMove == true) {
			canMove = false;
			StartCoroutine(MoveInGrid(xInt, yInt, zInt - gridSize));
			AddStepCount();
		}
